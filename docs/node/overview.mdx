...
**First time using fork mode?** Here are some quick tips to ensure success:

1. **Start with a working RPC URL**: Use a reliable provider like Alchemy, Infura, or QuickNode
2. **Use a recent but stable block**: Don't fork from the very latest block (use one that's 10-20 blocks old)
3. **Test with a simple example first**: Before complex scenarios, verify your fork works with a basic configuration
4. **Check your API key limits**: Fork mode can use significant RPC calls during startup

```typescript
// Minimal working fork setup for testing
const test = createOnchainTest(
  configure()
    .withLocalNode({
      fork: 'https://eth-mainnet.g.alchemy.com/v2/your-api-key',
      // Start simple - add more options as needed
    })
    .withMetaMask()
    .build()
);
